 ...
 *
 * $Id: cameliatk.jp.calendar.js,v 1.2 2007/10/16 13:39:34 jury Exp $
 * <@(#) $RCSfile: cameliatk.jp.calendar.js,v $ $Revision: 1.2 $>
 * 
 * 更新履歴
 * 2007/10/11 (岡村 淳司) 新規作成 cameliaTk.jp(Java)より移植
 */
...
/*
 * [object] このクラスは日付入力のためのWINDOWを制御します。
 */
...
/*
 * [function] 現在日付をYYYY/MM/DD形式で返します。
 * 
 */
function getToday() {
	today = new Date();
	yy = today.getFullYear();
	mm = zeroPadding(today.getMonth() + 1);
	dd = zeroPadding(today.getDate());
...
/*
 * [function] テキストボックスが空である場合、現在日付をセットします。
 * 
 * @param field テキストボックスコンポーネント
 */
function setTodaySingle(field) {
 	if (isEmptyTextBox(field)) {
		field.value = getToday(field.value)
 	}
}

/*
 * [function] 二つのテキストボックスが共に空である場合、現在日付を共にセットします。
 * 
 * @param field1 テキストボックスコンポーネント1
 * @param field2 テキストボックスコンポーネント2
 */
function setTodayDouble(field1, field2) {
	if (isEmptyTextBox(field1) && isEmptyTextBox(field2)) {
		setTodaySingle(field1);
		setTodaySingle(field2);
	}
}

/*
 * [function] 指定されたフィールドがテキストボックスでかつ空かどうかを判定します。
 * 
 * @param field テキストボックスコンポーネント
 */
function isEmptyTextBox(field) {
 	if (field.type == 'text' ) {
...
/*
 * [function] 日付用テキストボックスに本日日付をセットする、またはクリアします。
 * 
 * @param field テキストボックスコンポーネント
 */
function dateControlSingle(field) {
	if (isEmptyTextBox(field)) {
		setTodaySingle(field);
	}else{
		field.value = '';
	}
}
 ...
 * [function] 日付用テキストボックスに本日日付をセットする、またはクリアします。
 * 
 * @param field1 テキストボックスコンポーネント（主）
 * @param field2 テキストボックスコンポーネント（従）
 */
function dateControlDouble(field1, field2) {
	if (isEmptyTextBox(field1) && isEmptyTextBox(field2)) {
		setTodaySingle(field1);
		setTodaySingle(field2);
	}else{
