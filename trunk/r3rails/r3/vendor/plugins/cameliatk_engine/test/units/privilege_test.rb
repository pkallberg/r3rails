...
#
# <@(#) $Id: privilege_test.rb,v 1.3 2007/10/14 10:26:48 jury Exp $>
#
# 改定履歴
...
class RoleTest < Test::Unit::TestCase
  ...
  def test_allow
    ...
    privilege = privileges(:admin1001)
    assert privilege.allow?(url)
    assert !privilege.allow?()
    assert !privilege.allow?(nil)
    assert !privilege.allow?({})
    assert !privilege.allow?({:bad=>""})
  end
